...
Зависимости: только контракты (pydantic) и `ui.citations_view.format_citation`
(единая точка форматирования цитат — тот же вид, что на экране). Инвариант:
никакой бизнес-логики и обращений к стенду — форматирование уже полученных
данных; время формирования передаётся снаружи (generated_at), модуль не
трогает системные часы сам (детерминированность для тестов).
"""
...
# Входные связи: contracts.Answer; generated_at — строка даты от вызывающего
