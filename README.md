...
- **Фильтры в сайдбаре UI**: география — по признаку описываемого опыта в
  документе (не по языку текста — 158/177 документов русскоязычны, но описывают в т.ч.
  зарубежные практики), год — фильтрует цитаты чата по `Citation.year`; для подграфа
  ответа — типы объектов (8 типов онтологии), минимальная уверенность связи
  (противоречия показываются всегда) и размер схемы. Фильтры применяются к уже
  загруженным данным — переключение мгновенно, без повторных запросов к Neo4j.
- **Экспорт в Markdown** — кнопки «Скачать отчёт (.md)» у ответа чата (текст, источники,
  противоречия) и «Скачать карту пробелов (.md)» (таблица ячеек + темы only_ru/only_foreign) —
  готовый фрагмент для вставки в презентации и технические задания.
...
- **Ручная правка эксперта** (У-4) — версионирование-минимум на узлах/связях графа:
  ...
  Neo4j Browser (`http://localhost:7474`), без кода.

## Полный пайплайн end-to-end

Ниже — реальные точки входа модулей (проверено по факту в исходниках, аргументы CLI —
argparse каждого модуля). Порядок соответствует потоку данных
пайплайна. Neo4j в репозитории **уже наполнен** этим пайплайном —
