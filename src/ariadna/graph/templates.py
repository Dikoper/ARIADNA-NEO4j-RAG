...
`fetch_subgraph()` (A-12, интерфейсный контракт для UI/A-13) — второй публичный
вход модуля, не завязан на QueryIntent/TEMPLATES: подграф ответа/аналитики по
готовому набору Entity.id -> тоже plain `dict` {"nodes", "edges"}, см. её
пре-комментарий.
"""
...
from ariadna.graph.cypher_templates import (
    SUBGRAPH_EDGES_QUERY,
    SUBGRAPH_NODES_QUERY,
    TEMPLATE_DEFAULT_CANONICALS,
    TEMPLATES,
)
...
# ─── fetch_subgraph ─────────────────────────────────────────────────────────
# Назначение: подграф для визуализации (A-12, интерфейсный контракт для UI/A-13)
#   — узлы по Entity.id, урезанные до max_nodes по n_mentions (SUBGRAPH_NODES_
#   QUERY), и рёбра всех 6 типов онтологии МЕЖДУ УЖЕ УРЕЗАННЫМ набором узлов
#   (не исходным node_ids) — рёбра на отрезанный узел не тянут его обратно.
#   Пустой node_ids -> пустой подграф без обращения к Neo4j.
# Входные связи: neo4j.Driver (только чтение), node_ids: list[str] (Entity.id)
# Выходные данные: dict{"nodes": [{"id","name","type","is_tech_solution"}],
#   "edges": [{"source","target","type","confidence","is_contradicts"}]}
# Уровень: ✅ реализовано (A-12, worklogs/analytics.md)
def fetch_subgraph(driver: Driver, node_ids: list[str], *, max_nodes: int = 60) -> dict:
    if not node_ids:
        return {"nodes": [], "edges": []}
    nodes = _run(driver, SUBGRAPH_NODES_QUERY, {"node_ids": node_ids, "max_nodes": max_nodes})
    kept_ids = [row["id"] for row in nodes]
    edges = _run(driver, SUBGRAPH_EDGES_QUERY, {"node_ids": kept_ids}) if kept_ids else []
